...
namespace System.Caching
{
	...
	abstract class MemoryMonitor
	{
	    ...
	    static this() {
	        ...
	        if (UnsafeNativeMethods.GlobalMemoryStatusEx(&memoryStatusEx) != 0) {
	            s_totalPhysical = memoryStatusEx.ullTotalPhys;
	            s_totalVirtual = memoryStatusEx.ullTotalVirtual;
	        }
	    }

	    public static int64 TotalPhysical { get { return s_totalPhysical; } }
	    public static int64 TotalVirtual { get { return s_totalVirtual; } }

	    public int PressureLast { get { return _pressureHist[_i0]; } }
	    public int PressureHigh { get { return _pressureHigh; } }
	    public int PressureLow { get { return _pressureLow; } }
	    ...
	    protected void InitHistory() {
	        Runtime.Assert(_pressureHigh > 0, "_pressureHigh > 0");
	        Runtime.Assert(_pressureLow > 0, "_pressureLow > 0");
	        Runtime.Assert(_pressureLow <= _pressureHigh, "_pressureLow <= _pressureHigh");
	...
	sealed class CacheMemoryMonitor : MemoryMonitor, IDisposable
	{
		...
		public void SetLimit(int cacheMemoryLimitMegabytes)
		{
		    ...
		    // never override what the user specifies as the limit; only call AutoPrivateBytesLimit when the user does not specify one.
	...
	sealed class PhysicalMemoryMonitor : MemoryMonitor
	{
	    ...
	    // Returns the percentage of physical machine memory that can be consumed by an application before ASP.NET starts forcibly removing items from the cache.
	    public int64 MemoryLimit
		{
	    ...
	    public override int GetPercentToTrim(DateTime lastTrimTime, int lastTrimPercent)
		{
	        ...
	        if (IsAboveHighPressure()) {
	            // choose percent such that we don't repeat this for ~5 (TARGET_TOTAL_MEMORY_TRIM_INTERVAL) minutes, but keep the percentage between 10 and 50.
