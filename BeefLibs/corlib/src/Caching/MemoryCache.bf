...
namespace System.Caching
{
	sealed class MemoryCacheStore : IDisposable
	{
	    ...
	    public MemoryCacheEntry AddOrGetExisting(String key, MemoryCacheEntry entry)
		{
	        ...
	        // Call Release after the new entry has been completely added so that the CacheItemRemovedCallback can take a dependency on the newly inserted item.
	    ...
	    public void Dispose()
		{
	        if (Interlocked.Exchange(ref _disposed, 1) == 0) {
	            ...
	            // MemoryCacheStatistics has been disposed, and therefore nobody should be using _insertBlock except for potential threads in WaitInsertBlock (which won't care if we call Close).
	...
	*/
	...
	class MemoryCacheEntry //: MemoryCacheKey
	{
		...
		public this(String key, Object value, DateTimeOffset absExp, TimeSpan slidingExp, CacheItemPriority priority, List<ChangeMonitor> dependencies, CacheEntryRemovedCallback removedCallback, MemoryCache cache)
		{
	...
	public class MemoryCache : ObjectCache, IEnumerable<(String key, Object value)>, IDisposable
	{
		        ...
		        switch (reason) {
		            ...
		            case CacheEntryRemovedReason.ChangeMonitorChanged:
		                if (entry.ExpensiveObjectDependency.HasChanged)
		                    // If the expensiveObject has been removed explicitly by Cache.Remove, return from the SentinelEntry removed callback thus effectively removing the SentinelEntry from the cache.
		                    return;
		...
		{
		    get {
		        MemoryCacheStore[] allStores = new MemoryCacheStore[_storeCount];
		...
		{
		    Runtime.Assert(policy.AbsoluteExpiration == ObjectCache.InfiniteAbsoluteExpiration || policy.SlidingExpiration == ObjectCache.NoSlidingExpiration);
		    Runtime.Assert(policy.SlidingExpiration > ObjectCache.NoSlidingExpiration || OneYear >= policy.SlidingExpiration);
		    Runtime.Assert(policy.RemovedCallback == null || policy.UpdateCallback == null);
		    Runtime.Assert(policy.Priority >= CacheItemPriority.Default && policy.Priority <= CacheItemPriority.NotRemovable);
		}
		...
		private void Init()
		{
		    ...
		    _storeRefs = new MemoryCacheStore[_storeCount];
			_useMemoryCacheManager = true;

	        for (int i = 0; i < _storeCount; i++)
	            _storeRefs[i] = new MemoryCacheStore(this);

	        _stats = new MemoryCacheStatistics(this);
		}
		...
		private Object AddOrGetExistingInternal(String key, Object value, CacheItemPolicy policy)
		{
		    ...
		    MemoryCacheEntry entry = store.AddOrGetExisting(key, new MemoryCacheEntry(key, value, absExp, slidingExp, priority, changeMonitors, removedCallback, this));
		...
		public override void Set(String key, Object value, CacheItemPolicy policy, String regionName = null)
		{
		    ...
		    MemoryCacheStore store = GetStore(key);
		    store.Set(key, new MemoryCacheEntry(key, value, absExp, slidingExp, priority, changeMonitors, removedCallback, this));
		}
		...
		public void Set(String key, Object value, List<ChangeMonitor> changeMonitors, DateTimeOffset absoluteExpiration, TimeSpan slidingExpiration, CacheEntryUpdateCallback onUpdateCallback)
		{
		    ...
		    MemoryCacheEntry sentinelCacheEntry = new MemoryCacheEntry(sentinelCacheKey,
	...
	sealed class MemoryCacheStatistics : IDisposable
	{
		...
		public this(MemoryCache memoryCache)
		{
			...
			_timer = new PeriodicCallback(new => CacheManagerTimerCallback, _configPollingInterval);
		}
