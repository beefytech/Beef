...
namespace System
{
	...
	public static class Math
	{
		...
		{
			if ((digits < 0) || (digits > cMaxSingleRoundingDigits))
				Runtime.FatalError();
	...
	class MathTests
	{
		...
		{
			// Should succeed for digits 0-6
			float val = 3.14159f;
			float r0 = Math.Round(val, 0);
			Test.Assert(r0 == 3.0f);

			float r2 = Math.Round(val, 2);
			Test.Assert(Math.Abs(r2 - 3.14f) < 0.001f);

			float r6 = Math.Round(val, 6);
			Test.Assert(Math.Abs(r6 - 3.14159f) < 0.0001f);
		}

		[Test]
		public static void Round_Float_MidpointMode()
		{
			float val = 2.5f;
		...
		[Test]
		public static void Round_Double_HighDigits()
		{
			// Doubles should still accept up to 15 digits
			double val = 3.141592653589793;
			double r15 = Math.Round(val, 15);
			Test.Assert(Math.Abs(r15 - val) < 1e-15);
		}
	}
#endif
}
